...
- **Get the app banner (2026-05-08)**: Shown on `/`, `/login`, `/signup`, `/dashboard` unless user dismissed (`localStorage` `stackpilot:get_the_app:dismissed:v2`) or page is opened as installed PWA (standalone). Implemented as **`next/dynamic` + `ssr: false`** (`GetTheAppBanner.tsx` → `GetTheAppBannerInner.tsx`) so it always runs client-side after load. On `/`, the card sits **above the hero CTAs**. If you do not see it: load with **`?installHint=1`** (clears dismiss for that device) or remove the `localStorage` key. Production only shows it after the feature is **merged to `master` and deployed** (Vercel production must include these files).
