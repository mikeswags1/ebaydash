...
|------|-------|---------------|-----------|
| 2026-05-10 | Codex | Added the browser-installed "Get App" flow: homepage and desktop dashboard now show steps for iPhone Safari / Android Chrome Add to Home Screen, compact/PWA shell stays clean, homepage nav links to the section, and `public/sw.js` is restored as a minimal asset-only service worker for PWA install support. Verified with `npm run lint`, `npm run typecheck`, `npm run build`, and local `/` + `/sw.js` checks. | `app/components/GetAppBanner.tsx`, `app/page.tsx`, `app/dashboard/page.tsx`, `app/globals.css`, `public/sw.js` |
